...
from core.commandtypes import CommandPair
from core.callbacks import Callbacks
from core.utils import frame_probe_fast
from core.vapoursynth import compose_vapoursynth_pipe
...
def find_aom_keyframes(stat_file, key_freq_min, cb: Callbacks):
    keyframes_list = []
...
def aom_keyframes(video_path: Path, stat_file, min_scene_len, ffmpeg_pipe, video_params, is_vs, cb: Callbacks):
    """[Get frame numbers for splits from aomenc 1 pass stat file]
    """

    cb.run_callback("log", f'Started aom_keyframes scenedetection\nParams: {video_params}\n')
    ...
    cb.run_callback("newtask", "AOMKF First Pass Scenedetection", total)
    ...
    keyframes = find_aom_keyframes(stat_file, min_scene_len, cb)
